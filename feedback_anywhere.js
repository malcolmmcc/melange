...
var title = scr.getAttribute("data-title") || document.title;

var randId = Math.floor(10000000*Math.random());
var currentRating = 0;
function sendFeedback(rid) {
  var mailto = "mailto: " + emailAddress;
  mailto += '?subject=Blog post feedback for "' + title + '"';
  mailto += "&body=" + location.href + "%0A%0A";
  mailto += "Rating: "
...
if (includeRating) {
  ...
  var css = ".feedback_oneliner {width: 100%}";
  if (!document.getElementById("feedback_stars")) {
    css += ".feedback_star_rating {width: 100px;text-align:left;display: inline-block;unicode-bidi: bidi-override;direction: rtl}"
...
var html = '<input type="text" class="feedback_oneliner" placeholder="...type here..." id="feedback_oneliner_' + randId + '" tabindex="100" />';
